 ...
 **/
static Double[] A = {62.32, 1.0, 40.34, 60.75, 3.9, 79.9, 49.54, 90.11, 68.54, 97.99};// identify an arr
...
private static  <E> void quickSort4 (Double[] s, Comparator<Double> c) {
  datasetRow=0;
	if (s.length < 2) return; // the array is already sorted in this case
  quickSortStep4(s, c, 0, s.length-1); // recursive sort method
}
...
public static  void quickSortStep4 (Double[] s, Comparator<Double> c,
                            int leftBound, int rightBound ) {
  if (leftBound >= rightBound) return; // the indices have crossed
  Double temp;  // temp object used for swapping
  Double pivot= s[rightBound];
  ...
  while (leftInd <= rightInd) { // scan right until larger than the pivot, erreur injectee initialement while (leftInd <= rightInd)
    ...
    //  Random r = new Random();
      ...
      //if ((datasetRow<3))
     {
    	    ...
    	    for (int i = 0; i <7; i++) {
    	      int z = r.nextInt(100);
    	      s[i] =  new Double(i*7+z);
    	     //System.out.println("I AM HERE");
    	      
    	    }
      }
      for (int f=0;f<s.length;f++) writenew(s[f].toString());
     if ((s[rightInd]==templeft)&&(s[leftInd]==temp)) writenew("1");else writenew("0");
     writenew("\n");
  ...
  quickSortStep4(s, c, leftBound, leftInd-1); // left recursive call
  //out(" "+Arrays.asList(s));
  //out(" pivot: "+s[rightBound] );
 quickSortStep4(s, c, leftInd+1, rightBound); // right recursive call
  //out(" " + Arrays.asList(s));
}
...
public static void main (String[] argv) throws IOException {
	...
	start=true;
		...
		File file = new File("C:\\Users\\AliJaoua\\eclipse-workspace\\datastructures\\sort_100.csv");
//Transformer_B2_Experiment2 t1= new Transformer_B2_Experiment2();// to invoke an object from Transform class  
  ...
  Random r = new Random();
  Comparator<Double> c = new DefaultComparator<Double>();// C will be used to call compare function() 
 ...
 // String num = in.readLine();// to read input line
  ...
  int n=10;
  String cont;
  Double[] B=  {62.32, 1.0, 40.34, 60.75, 3.9, 79.9, 49.54, 90.11, 68.54, 97.99};
  //= {62, 1, 40, 60, 3, 79, 49, 90, 68, 97};B= {45, 14, 62, 29, 79, 73, 2, 45, 5, 12} ;
  //Integer[] B=new Integer[n];
  Double[] A = new Double[n];
  float msin=0f,qsin=0f,msout=0f,qsout=0f;
  long t;
    PositionList<Double> C = new NodePositionList<Double>();
    PositionList<Double> D = new NodePositionList<Double>();
    for (int i = 0; i < n; i++) {
      int x = r.nextInt(100);
      A[i] = new Double(x);
      B[i] = new Double(x);
      C.addLast(new Double(x));
      D.addLast(new Double(x));
    }
    ...
    //if (file.exists())file.delete();
    file = new File("C:\\Users\\AliJaoua\\eclipse-workspace\\datastructures\\sort_100.csv");
    t = System.currentTimeMillis();
    quickSort4(B, c);
