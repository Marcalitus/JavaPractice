  ...
  private static <E> void quickSortStep (E[] s, Comparator<E> c,
    ...
    while (leftInd <= rightInd) { // scan right until larger than the pivot
      while ( (leftInd <= rightInd) && (c.compare(s[leftInd], pivot)<=0) )// changed <= initially
        leftInd++; 
      while ( (rightInd >= leftInd) && (c.compare(s[rightInd], pivot)>=0))//changed >=initially
        rightInd--;
      if ((leftInd < rightInd)&&!(c.compare(s[rightInd], s[leftInd])==0)) { // both elements were found, so swap
