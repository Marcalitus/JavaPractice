...
public class sorting {
  ...
  public  <E> void mergeSort1(E[] orig, Comparator<E> c) { 
    E[] in = (E[]) new Object[100]; // make a new temporary array
    System.arraycopy(orig,0,in,0,100); // copy the input
    E[] out = (E[]) new Object[100]; // output array
    ...
    System.arraycopy(in,0,orig,0,100);
  }
  /** Merges two subarrays, specified by a start and increment. */
  protected  <E> void merge1(E[] in, E[] out, Comparator<E> c, int start, 
      int inc)
 { // merge in[start..start+inc-1] and in[start+inc..start+2*inc-1]
    int x = start; // index into run #1
    int end1 = Math.min(start+inc, 100); // boundary for run #1
    int end2 = Math.min(start+2*inc, 100); // boundary for run #2
  ...
  //  String num = in.readLine();
    ...
    long t;
      ...
      out("mergesort - Array-Based Sorting");
