...
static String sort_filename = "C:\\Users\\AliJaoua\\Documents\\QUTAIBAH-MEETING-7-7-2020\\NEWVERSION-Implications\\Sortcorrects2.csv";
...
public static  void quickSortStep3 (Integer[] s, Comparator<Integer> c,
  ...
  while (leftInd <= rightInd) { // scan right until larger than the pivot, erreur injectee initialement while (leftInd <= rightInd)
    ...
    { leftInd++;countcompare++; }// devrait etre leftInd=leftInd+1; 
   
    while ( (rightInd >= leftInd) && (c.compare(s[rightInd], pivot)>=0))
      {rightInd--;countcompare++;}

    int x;
    Random r = new Random();
    //x=r.nextInt(100)%2;
if ((leftInd < rightInd)&&!(c.compare(s[rightInd], s[leftInd])==0)) //Insertion of an error *****..
    { // both elements were found, so swap
      if ((rightInd>=0)&&(rightInd<=9)) temp = s[rightInd]; else temp=0;
      countcompare++;
      //***************************** error injected one time only
      int templeft;
      if (!inject) {templeft=  10000; inject=true;}else templeft=  s[leftInd]; //correct statements
    //***************************** error injected one time only
      
      if ((rightInd>=0)&&(rightInd<=9))s[rightInd] =templeft;
      if ((leftInd>=0)&&(leftInd<=9)) s[leftInd] = temp;
    ...
    // if ((s[rightInd]==templeft)&&(s[leftInd]==temp)) writenew("1");else writenew("0");
     ...
     // insertion of radom values in only one random cell in s
    
      
    }
  } // the loop continues until the indices cross
  
 if (!(c.compare(s[rightBound], s[leftInd])==0))  //insertion of another error ***********
  {/* Random xr = new Random();
  int indexc=xr.nextInt(9)+1;
 int xx = xr.nextInt(300);
  s[indexc] = new Integer(xx);*/
